...
export function move_a_rover(the_rover:Rover,the_plateau:Plateau) : string {
  ...
  if (validator_Rover(the_rover.Rover_x,the_rover.Rover_y,the_plateau) === false)
  return `Invalid rover coordinates are given : (${the_plateau.Plateau_x},${the_plateau.Plateau_y}). Please type-in new rover coordinates wherein the rovers must be initialized.`;
      ...
      if (the_rover.Rover_commands[i] === "M") {
        ...
        if (the_rover.Rover_facing === "N") {
          ...
          if (validator_Rover(the_rover.Rover_x,the_rover.Rover_y+1,the_plateau) === false ) {
            return `The rover has stopped on location : ${the_rover.Rover_x} ${the_rover.Rover_y} ${the_rover.Rover_facing} as the new coordinates will be out of bounds`;
          };
          the_rover.Rover_y += 1 ;
