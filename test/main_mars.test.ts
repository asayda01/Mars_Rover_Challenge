...
describe("throws an error if a plateau cannot be created with given upper-right coordinates.",() => {
    test (" if inputs are INvalid, it throws an error and asks for valid coordinates.",() => {
        ...
        const plat_2 = new Plateau (-5,-5);
        const rover_2 = new Rover (-3,-4,"S","LMLMRMRMLMLMRM");
...
describe("throws an error if a there`s a geographic problem with starting coordinates of rovers.",() => {
    test ("if inputs are valid but rovers are initialized outside of a given plateau, it throws an error and asks for valid coordinates wherein the rovers must be.",() => {
        const plat_4 = new Plateau (5,5);
        const rover_4 = new Rover (7,9,"S","MLMMLRMLRRM");
        expect ( move_a_rover(rover_4,plat_4)).toBe(`Invalid rover coordinates are given : (${plat_4.Plateau_x},${plat_4.Plateau_y}). Please type-in new rover coordinates wherein the rovers must be initialized.`)});
    test ("if inputs are valid but rovers cannot move on a given plateau, it throws an error and asks for valid upper-right coordinates which allow rovers to explore.",() => {
        const plat_5 = new Plateau (5,5);
        const rover_5 = new Rover (5,5,"E","MMRMMRMRRM");
        expect ( move_a_rover(rover_5,plat_5)).toBe(`The rover has stopped on location : ${plat_5.Plateau_x} ${plat_5.Plateau_y} ${rover_5.Rover_facing} as the new coordinates will be out of bounds`);
    })}
);
...
describe("Moves rovers and returns their new coordinates on a plateau.",() => {
    ...
    test ("if inputs are valid but rovers have reached edges of a plateau, it must stop the rovers to prevent them from leaving a plateau and inform the status of rovers.",() => {
